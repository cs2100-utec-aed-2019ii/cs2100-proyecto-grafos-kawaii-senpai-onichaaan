...
private:
	...
	static GLvoid window_display() {
		...
		glOrtho(0.0f, limits.x, limits.y, 0.0f, 0.0f, limits.z);
	...
	static GLvoid callback_mouse(int button, int state, int x, int y) {
		if (state == GLUT_DOWN && button == GLUT_LEFT_BUTTON) universe.trigger("onLeftClick", (void*)& coord::create(x, y));
		else if (state == GLUT_DOWN && button == GLUT_RIGHT_BUTTON) universe.trigger("onRightClick", (void*)& coord::create(x, y));
		else if (state == GLUT_DOWN && button == GLUT_MIDDLE_BUTTON) universe.trigger("onMiddleDown", (void*)& coord::create(x, y));
		else if (state == GLUT_UP && button == GLUT_MIDDLE_BUTTON) universe.trigger("onMiddleUp", (void*)& coord::create(x, y));
	}
